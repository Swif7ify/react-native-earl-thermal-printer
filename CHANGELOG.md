...
- `tailingLine: false` was still appending trailing blank lines due to a faulty type check. It now correctly produces no trailing whitespace when set to `false`.

## [1.1.0] - 2025-02-18

### Added

- `imageWidth` parameter to `printImage()` — allows setting the maximum width (in pixels) for printed images. Defaults to `200` on Android and `150` on iOS.
- `qrSize` parameter to `printQrCode()` — allows setting the size (in pixels) of the generated QR code. Defaults to `250`.
